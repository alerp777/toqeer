...
router.post("/products", validateBody(productCreateSchema), async (req, res, next) => {
  try {
    const vendorId = req.vendorId!;
    const user = req.vendorUser!;
    const {
...
router.post("/wallet/withdraw", paymentLimiter, validateBody(vendorWithdrawSchema), async (req, res, next) => {
  try {
